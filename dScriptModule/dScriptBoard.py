...
class dScriptBoard(dScriptObject):
    ...
    def __init__(self, TCP_IP, TCP_PORT=17123, PROTOCOL='binary'):
        #_LOGGER.debug("dScriptBoard: __init__")
        self.IP = TCP_IP
        self.Port = TCP_PORT
    ...
    def __SendProtocol(self,command,arguments):
        _LOGGER.debug("dScriptBoard: __SendProtocol: %s | %s",command, arguments)
    ...
    def _AESEncrypt(self,msg):
        #_LOGGER.debug("dScriptBoard: _AESEncrypt: %s", msg)
        #_LOGGER.debug("dScriptBoard: extend msg to 12 Bytes: %s", msg)
        while len(msg) < 12:
            msg = msg + struct.pack("B",0)
        #_LOGGER.debug("dScriptBoard: msg extended: %s", msg)
        if msg[0] == self._AESNonceInitCMD:
            #_LOGGER.debug("dScriptBoard: %s: Initialize Nonce command", self._HostName)
            ...
            #_LOGGER.debug("dScriptBoard: %s: Command requiring Nonce", self._HostName)
            if not self._Nonce:
                _LOGGER.info("dScriptBoard: %s: Need to (re)initial Nonce value", self._HostName)
                self.GetStatus()
            if not self._Nonce:
                _LOGGER.error("dScriptBoard: %s: Could not receive Nonce", self._HostName)
                return False
            msg = msg + self._Nonce
            self._Nonce = '' # reset nonce to prevent duplicate usage
        else:
            #_LOGGER.debug("dScriptBoard: %s: Command not using Nonce", self._HostName)
            msg = msg + struct.pack("B",0) + struct.pack("B",0) + struct.pack("B",0) + struct.pack("B",0) # just add 0 for unneeded
        #_LOGGER.debug("dScriptBoard: msg to encrypt: %s", msg)
        iv = Random.new().read(AES.block_size)
        cipher = AES.new(self._AESKey, AES.MODE_CBC, iv)
        msg = iv + cipher.encrypt(msg)
        #_LOGGER.debug("dScriptBoard: msg encrypted: %s", msg)
        return msg 

    '''Decrypt a message using AES encryption'''
    def _AESDecrypt(self,data,binaryid):
        #_LOGGER.debug("dScriptBoard: _AESDecrypt: %s", data)
        ...
        if self._IsInList(binaryid, self._AESNonceCommands):
            _LOGGER.debug("dScriptBoard: %s: Update Nonce value: %s", self._HostName, data[12:])
            self._Nonce = data[12:]
        #_LOGGER.debug("dScriptBoard: data decrypted: %s", data)
        return data

    '''Send a message to the board''' 
    def __Send(self,msg,buff):
        _LOGGER.debug("dScriptBoard: __Send: %s | %s",msg,buff)
    ...
    def __CheckIdentifier(self,identifier,idtype):
        #_LOGGER.debug("dScriptBoard: __CheckIdentifier")
        if idtype == 'light':
    ...
    def GetHostName(self):
        #_LOGGER.debug("dScriptBoard: GetHostName")
    ...
    def GetStatus(self):
        #_LOGGER.debug("dScriptBoard: GetStatus")
        data=self.__SendProtocol('GetStatus',[])
        databytes=self._ToDataBytes(data)
        databits=self._ToDataBits(data)

        _LOGGER.info("dScriptBoard: %s: Update board status information", self._HostName)
    ...
    def GetConfig(self):
        #_LOGGER.debug("dScriptBoard: GetConfig")
        try:
        ...
        if data[6] == 0: # data[6] is the number of physical arrays - this should never be 0
            _LOGGER.info("dScriptBoard: %s: Contains default firmware", self._HostName)
            self._CustomFirmeware=False
            return False
        self._CustomFirmeware=True
        databytes=self._ToDataBytes(data)

        _LOGGER.info("dScriptBoard: %s: Update board config information", self._HostName)
    ...
    #    #_LOGGER.debug("dScriptBoard: GetLight: %s",identifier)
    ...
    #    databytes=self._ToDataBytes(data)
    #    _LOGGER.info("dScriptBoard: %s: Light %s is %s", self._HostName, identifier, self._OnOffStates[databytes[0]])
    ...
    def GetShutter(self,identifier):
        #_LOGGER.debug("dScriptBoard: GetShutter: %s",[identifier])
        if not self._CustomFirmeware:
            return False
        ...
        databytes=self._ToDataBytes(data)
        _LOGGER.info("dScriptBoard: %s: Shutter %s is %s at level %s%%", self._HostName, identifier, self._ShutterStates[databytes[1]], databytes[0])
    ...
    def GetSocket(self,identifier):
        #_LOGGER.debug("dScriptBoard: GetSocket: %s",[identifier])
        if not self._CustomFirmeware:
            return False
        ...
        databytes=self._ToDataBytes(data)
        _LOGGER.info("dScriptBoard: %s: Socket %s is %s", self._HostName, identifier, self._OnOffStates[databytes[0]])
    ...
    def GetMotion(self,identifier):
        _LOGGER.debug("dScriptBoard: GetMotion: %s",[identifier])
        if not self._CustomFirmeware:
            return False
        ...
        databytes=self._ToDataBytes(data)
        _LOGGER.info("dScriptBoard: %s: Motion Sensor %s is %s", self._HostName, identifier, self._OnOffStates[databytes[0]])
    ...
    def GetButton(self,identifier):
        _LOGGER.debug("dScriptBoard: GetButton: %s",[identifier])
        if not self._CustomFirmeware:
            return False
        ...
        databytes=self._ToDataBytes(data)
        _LOGGER.info("dScriptBoard: %s: Button %s was clicked %s times", self._HostName, identifier, databytes[0])
    ...
    def SetRelay(self,identifier,state):
        #_LOGGER.debug("dScriptBoard: SetRelay: %s | %s",identifier,state)
    ...
    def SetLight(self,identifier,state):
        #_LOGGER.debug("dScriptBoard: SetLight: %s | %s",identifier,state)
    ...
    def SetShutter(self,identifier,state):
        #_LOGGER.debug("dScriptBoard: SetShutter: %s | %s",identifier,state)
        if not self._CustomFirmeware: # fallback to legacy relay action
            return self.SetRelay(identifier,state)
        if not self.__CheckIdentifier(identifier,'shutter'):
            return False
    ...
    def SetSocket(self,identifier,state):
        #_LOGGER.debug("dScriptBoard: SetSocket: %s | %s",identifier,state)
        if not self._CustomFirmeware: # fallback to legacy relay action
            return self.SetRelay(identifier,state)
        if not self.__CheckIdentifier(identifier,'socket'):
            return False
